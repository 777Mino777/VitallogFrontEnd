...
const InformationPage = () => {
  ...
  const calcBmi = (height, weight) => {
    ...
    const checkBmi = weight / (heightInMeters * heightInMeters);
  ...
  return (
    <div className="max-w-[1820px] mx-auto">
      <div className="flex flex-col min-h-screen bg-white text-gray-800">
        ...
        <h1 className="mx-16 pb-8 pt-16 text-gray-400 font-bold">선택하지 않은 경우 모든 옵션을 포함하는 정보를 볼 수 있습니다.</h1>
        <div className="form flex justify-center pb-20">
          <table className="flex justify-around">
            <thead className="px-10 space-x-32">
              <select className='border-4 font-bold w-25 px-20 py-6 text-2xl rounded-2xl border-black transition duration-300 hover:border-custom-blue' value={gender} onChange={(e) => setGender(e.target.value)}>
        ...
        <div className='mx-14 flex pb-10 text-3xl justify-start space-x-80'>
          <section className='border'>성별 : {}</section>
          <section className='border'>나이 : {}</section>
          <section className='border'>BMI : {}</section>
        </div>
        <table className="border border-black text-3xl ml-14 mr-14 text-center">
          <thead>
            <tr>
